...
function fixRowspan(id){
	$(".deviceList").each(function(){
		var $t = $(this);
		var $trs = $t.find("tr");
		var cols = $trs.eq(0).find("td").length;
		var rows = $trs.length;

		for(i = 0; i < cols; i++){
			for(j = 1; j < rows; j++){
				var $td = $trs.eq(j).find("td").eq(i);
				...
				rs = $td.attr("rowspan") * 1;
				if(rs > 1){
					$td.removeAttr("rowspan");
					$td.css({"background" : "#18b9d4"});
					//$td.text(rs);
					for(y = 1; y < rs; y++){
						var newTd = $("<td>", {"text" : text, "style" : "background : #ebeba2"});
						if(i > 0){
							var $p = $trs.eq(j+y).find("td").eq(i-1);
							newTd.insertAfter($p);
						}else{
							$trs.eq(j+y).prepend(newTd);
						}
					}
				}
			}
	...
	$(".deviceList").each(function(j){
		...
		$tr.each(function (i) {
			...
			var $form = $("<form>", {
				...
				, "method": "post"
				, "ajax": true
				, "ajax-loader-id": "cart_list_" + id + "-" + i + "-" + j
			});
			var $code = $("<input>", {
				"name" : "prod"
...
$(document).on("click", ".partners span", function(){
	var href = "http://" + $(this).attr("data-link");
