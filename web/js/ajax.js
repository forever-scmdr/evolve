 ...
 * @additionalHandling - дополнительная функция, которая вызывается после успешного завершения всех действий по обработке запроса
 */
function postFormAjax(formId, lockElementIds, additionalHandling) {
	var ok = true;
	ipt = $('#' + formId).find('label.required').find('input');
	ipt.change(function(){$(this).css({borderColor:''});});
	ipt.each(function(i){
	...
	if(!ok){alert("Пожалуйста, заполните обязательные поля.\nОни отмечены символом ' * '.");}
	if(ok){
	$('#' + formId).ajaxSubmit({
		error: function() {
			alert('Ошибка отправки формы');
...
function postFormJquery(form, lockElementIds, additionalHandling) {
	if (typeof form == 'string')
		form = $('#' + form);
	$.ajax({
		type: "POST",
...
//-- Ajax link declaration and popup implementation
