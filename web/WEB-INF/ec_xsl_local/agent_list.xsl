...
<xsl:text disable-output-escaping="yes">&lt;!DOCTYPE html&gt;</xsl:text>
	...
	<xsl:variable name="agent_branches" select="page/branches"/>
	<xsl:variable name="agent_types" select="page/types"/>

	<xsl:variable name="branches_all" select="page/branches_all/string_element"/>
	<xsl:variable name="types_all" select="page/types_all/string_element"/>
	
	<xsl:variable name="agents" select="page/agent"/>
	<xsl:variable name="countries" select="distinct-values($agents/country)"/>
	...
	<div class="menu popup" style="display: none" id="form_{@id}">
		...
		<form method="post" action="{submit}" style="white-space: nowrap; margin-bottom: 20px">
			<div class="coloumn">
				...
				<select id="edit_type_{@id}" name="{type/@input}" value="{type}">
					<xsl:for-each select="$types_all"><option><xsl:value-of select="name"/></option></xsl:for-each>
				</select>
				<label for="edit_branch_{@id}">Отрасль:</label>
				<select id="edit_branch_{@id}" name="{branch/@input}" value="{branch}">
					<xsl:for-each select="$branches_all"><option><xsl:value-of select="name"/></option></xsl:for-each>
				</select>
			...
			<div class="coloumn" style="margin-right: -20px;">
				<label for="edit_email_{@id}">Эл. почта основная:</label>
				<input id="edit_email_{@id}" type="text" name="{email/@input}" value="{email}"/>
				<label for="edit_email_{@id}">Эл. почта 2:</label>
				<input id="edit_email_{@id}" type="text" name="{email_2/@input}" value="{email_2}"/>
				<label for="edit_email_{@id}">Эл. почта 3:</label>
				<input id="edit_email_{@id}" type="text" name="{email_3/@input}" value="{email_3}"/>
				...
				<label for="edit_boss_name_dat_{@id}">Руководитель в дательном:</label>
				<input id="edit_boss_name_dat_{@id}" type="text" name="{boss_name_dat/@input}" value="{boss_name_dat}"/>
				<label for="edit_boss_position_{@id}">Должность руководителя:</label>
				<input id="edit_boss_position_{@id}" type="text" name="{boss_position/@input}" value="{boss_position}"/>	
				<label for="edit_boss_position_dat_{@id}">Должность в дательном:</label>
				<input id="edit_boss_position_dat_{@id}" type="text" name="{boss_position_dat/@input}" value="{boss_position_dat}"/>
				<label for="edit_boss_greetings_{@id}">Обращение к руководителю:</label>
				<input id="edit_boss_greetings_{@id}" type="text" name="{boss_greetings/@input}" value="{boss_greetings}"/>
			</div>
			<br/>
			<br/>
	...
	<div class="menu popup" style="display: none;" id="popup_send_email">
		...
		<p style="position: relative; top: -15px; font-size: 13px;">
			Выбрано контрагентов: <span class="email_agents_qty">0</span>
		</p>
		<form method="post" action="{page/add_email_to_queue_no_resend}" target="_blank">
			...
			<br/>
			<br/>
			<input type="submit" value="Отправить письма" class="buttonSubmit" 
				onclick="$(this).closest('.popup').hide('fade', 200)"/>
			<input type="submit" value="Отправить письма повторно" class="buttonSubmit" 
				onclick="$(this).closest('form').attr('action', '{page/add_email_to_queue_resend}'); $(this).closest('.popup').hide('fade', 200)"/>
		...
		<body>
			...
			<div class="mainContainer">
				...
				<div class="tools">
					...
					<a class="buttonSubmit" href="#" popup="popup_send_email">Рассылка (<span class="email_agents_qty">0</span>)</a>
					...
					<a class="buttonSubmit" href="#" onclick="$('#delete_all_selected_form').submit(); return false;">Удалить (<span class="email_agents_qty">0</span>)</a>
					<form method="post" action="{page/delete_all}" id="delete_all_selected_form" style="display: none">
						<input type="hidden" name="id" value="" id="delete_agent_ids"/>
					</form>
					<div class="search">
				...
				<div class="menu popup dropdown" style="top: 80px; left: 10px; display: none" id="popup_cols">
					...
					<form action="{page/cols_base_link}" method="post">
						<div class="coloumn">
							...
							<input type="checkbox" name="cols" id="cb_boss_name_dat" value="boss_name_dat">
								<xsl:if test="'boss_name_dat' = $cols"><xsl:attribute name="checked" select="'checked'"/></xsl:if>
							</input>
							<label for="cb_boss_name_dat">Руководитель в дательном</label><br/>

							<input type="checkbox" name="cols" id="cb_boss_position" value="boss_position">
								<xsl:if test="'boss_position' = $cols"><xsl:attribute name="checked" select="'checked'"/></xsl:if>
							</input>
							<label for="cb_boss_position">Должность руководителя</label><br/>

							<input type="checkbox" name="cols" id="cb_boss_position_dat" value="boss_position_dat">
								<xsl:if test="'boss_position_dat' = $cols"><xsl:attribute name="checked" select="'checked'"/></xsl:if>
							</input>
							<label for="cb_boss_position_dat">Должность в дательном</label><br/>
							
							<input type="checkbox" name="cols" id="cb_boss_greetings" value="boss_greetings">
								<xsl:if test="'boss_greetings' = $cols"><xsl:attribute name="checked" select="'checked'"/></xsl:if>
							</input>
							<label for="cb_boss_greetings">Обращение к руководителю</label><br/>
					...
					<tr>
						...
						<xsl:if test="'country' = $cols">
							<th style="position: relative">
								...
								<xsl:variable name="min_width" select="ceiling(count($countries) div 10) * 220"/>
									...
									<form action="{page/country_base_link}" method="post">
										<div class="coloumn">
											...
											<xsl:for-each select="$countries">
												...
												<input type="checkbox" name="country" id="check_country_{position()}" value="{.}">
													<xsl:if test=". = $country"><xsl:attribute name="checked" select="'checked'"/></xsl:if>
												</input>
												<label for="check_country_{position()}"><xsl:value-of select="."/></label><br/>
											</xsl:for-each>
										</div>
										<br/>
										<input type="submit" value="Применить" class="buttonSubmit"/>
						...
						<xsl:if test="'region' = $cols">
							<th style="position: relative">
								...
								<xsl:variable name="min_width" select="4 * 220"/>
									...
									<form action="{page/region_base_link}" method="post">
										<div class="coloumn">
											...
											<xsl:for-each select="$countries">
												...
												<xsl:variable name="regions" select="distinct-values($agents[country = $c]/region)"/>
												<xsl:variable name="cpos" select="position()"/>
												<xsl:if test="position() mod 3 = 0 or (count($regions) &gt;= 5 and position() != 1)">
													<xsl:text disable-output-escaping="yes">&lt;/div&gt;&lt;div class="coloumn"&gt;</xsl:text>
												</xsl:if>
												<h4><xsl:value-of select="$c"/></h4>
												<xsl:for-each select="$regions">
													<xsl:sort select="."/>
													<xsl:if test="position() mod 10 = 0">
														<xsl:text disable-output-escaping="yes">&lt;/div&gt;&lt;div class="coloumn"&gt;</xsl:text>
													</xsl:if>
													<input type="checkbox" name="region" id="check_region_{$cpos}_{position()}" value="{.}">
														<xsl:if test=". = $region"><xsl:attribute name="checked" select="'checked'"/></xsl:if>
													</input>
													<label for="check_region_{$cpos}_{position()}"><xsl:value-of select="."/></label><br/>
												</xsl:for-each>
											</xsl:for-each>
										</div>
										<br/>
									...
									<form action="{page/city_base_link}" method="post">
										<div class="coloumn">
											...
											<xsl:for-each select="$countries">
												<xsl:sort select="."/>
												<xsl:variable name="c" select="."/>
												<xsl:variable name="cities" select="distinct-values($agents[country = $c]/city)"/>
												<xsl:if test="position() mod 3 = 0 or count($cities) &gt;= 5 and position() != 1">
													<xsl:text disable-output-escaping="yes">&lt;/div&gt;&lt;div class="coloumn"&gt;</xsl:text>
												</xsl:if>												
												<h4><xsl:value-of select="$c"/></h4>
												<xsl:for-each select="$cities">
													...
													<input type="checkbox" name="city" id="check_city_{position()}" value="{.}">
														<xsl:if test=". = $city"><xsl:attribute name="checked" select="'checked'"/></xsl:if>
													</input>
													<label for="check_city_{position()}"><xsl:value-of select="."/></label><br/>
												</xsl:for-each>
											</xsl:for-each>
										</div>
										<br/>
						...
						<xsl:if test="'boss_name_dat' = $cols">
							<th><a href="{f:set_url_param(page/sort_base_link, 'sort', 'boss_name_dat')}">Руководитель в дательном</a></th>
						</xsl:if>
						<xsl:if test="'boss_position' = $cols">
							<th><a href="{f:set_url_param(page/sort_base_link, 'sort', 'boss_position')}">Должность руководителя</a></th>
						</xsl:if>
						<xsl:if test="'boss_position_dat' = $cols">
							<th><a href="{f:set_url_param(page/sort_base_link, 'sort', 'boss_position_dat')}">Должность в дательном</a></th>
						</xsl:if>
						<xsl:if test="'boss_greetings' = $cols">
							<th><a href="{f:set_url_param(page/sort_base_link, 'sort', 'boss_greetings')}">Обращение к руководителю</a></th>
						</xsl:if>
									...
									<form action="{page/type_base_link}" method="post">
										<div class="coloumn">
											<xsl:for-each select="$agent_types">
												...
												<input type="checkbox" name="type" id="check_type_{position()}" value="{type}">
													<xsl:if test="type = $type"><xsl:attribute name="checked" select="'checked'"/></xsl:if>
												</input>
												<label for="check_type_{position()}"><xsl:value-of select="type"/></label><br/>
											</xsl:for-each>
										</div>
										<br/>
										<input type="submit" value="Применить" class="buttonSubmit"/>
									...
									<form action="{page/branch_base_link}" method="post">
										<div class="coloumn">
											<xsl:for-each select="$agent_branches">
												...
												<input type="checkbox" name="branch" id="check_branch_{position()}" value="{branch}">
													<xsl:if test="branch = $branch"><xsl:attribute name="checked" select="'checked'"/></xsl:if>
												</input>
												<label for="check_branch_{position()}"><xsl:value-of select="branch"/></label><br/>
											</xsl:for-each>
										</div>
										<br/>
										<input type="submit" value="Применить" class="buttonSubmit"/>
						...
						<tr>
							...
							<xsl:if test="'email' = $cols">
								<td>
									...
									<xsl:if test="email_2 and not(email_2 = '')"><br/><a href="mailto:{email_2}"><xsl:value-of select="email_2"/></a></xsl:if>
									<xsl:if test="email_3 and not(email_3 = '')"><br/><a href="mailto:{email_3}"><xsl:value-of select="email_3"/></a></xsl:if>
								</td>
							</xsl:if>
							<xsl:if test="'site' = $cols">
							...
							<xsl:if test="'boss_name_dat' = $cols">
								<td><xsl:value-of select="boss_name_dat"/></td>
							</xsl:if>
							<xsl:if test="'boss_position' = $cols">
								<td><xsl:value-of select="boss_position"/></td>
							</xsl:if>
							<xsl:if test="'boss_position_dat' = $cols">
								<td><xsl:value-of select="boss_position_dat"/></td>
							</xsl:if>
							<xsl:if test="'boss_greetings' = $cols">
								<td><xsl:value-of select="boss_greetings"/></td>
							</xsl:if>
							<xsl:if test="'contact_name' = $cols">
			...
			<script type="text/javascript">
				...
				$(document).ready(function() {
					...
					$('input.send_to_agent').click(function() {
