...
<xsl:stylesheet
	...
	<xsl:template match="/">
		<div class="result" id="{$feed2_ajax_id}">
			...
			<xsl:if test="$success">
				...
				<p><a onlick="insertAjax('{page/feedback_form_link2}', '{$feed2_ajax_id}'); return false;">Написать еще отзыв</a></p>
			</xsl:if>
		</div>
	</xsl:template>
	...
	<xsl:template name="FEEDBACK_2_BUTTON">
		...
		<a onclick="getForm2()" class="btn btn-success{' btn-block'[$block]}" type="button" data-toggle="modal" data-target="#fform2">
