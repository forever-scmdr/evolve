...
<xsl:stylesheet
		...
		<div class="result" id="{$subs_ajax_id}">
			<xsl:if test="not($success)">
				...
				<form method="post" action="{$f/submit_link}">
					...
					<button type="button" class="btn btn-info btn-block" data-dismiss="modal">Отписаться от всего</button>
				</form>
			</xsl:if>
			<xsl:if test="$success">
				<p>
	...
	<xsl:template name="SUBSCRIBE_BUTTON">
		...
		<a onclick="getSubscribeForm()" class="btn btn-success{' btn-block'[$block]}" type="button" data-toggle="modal" data-target="#subscribe">
