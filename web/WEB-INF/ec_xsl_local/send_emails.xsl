	...
	<xsl:template match="/">
		...
		<body>
			...
			<table>
				...
				<tr>
					<td>Отослано писем</td>
					<td class="error"><xsl:value-of select="/page/processed"/></td>
				</tr>
				<tr><td colspan="2" align="center"><b>Хронология выполнения</b></td></tr>
			...
			<xsl:if test="/page/error">
				<h2>ошибки отправки</h2>
				<table>
					<xsl:for-each select="/page/error">
						<tr>
							<td class="string-no">
							Источник: <span class="no"><xsl:value-of select="@line"/></span> 
							 Ошибка: <span class="no"><xsl:value-of select="@coloumn"/></span>
							</td>
							<td class="error"><xsl:value-of select="."/></td>
						</tr>
					</xsl:for-each>
