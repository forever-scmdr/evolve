...
public class Book_general_foreign
    extends Item
{

    private final static String _ITEM_TYPE_NAME = "book_general_foreign";

    private Book_general_foreign(Item item) {
        super(item);
    }

    public static Book_general_foreign get(Item item) {
        boolean isCompatible = ItemTypeRegistry.getItemPredecessorsExt(item.getTypeName()).contains(_ITEM_TYPE_NAME);
        if (!isCompatible) {
            throw new ClassCastException(("Wrapper 'book_general_foreign' can not be created around '"+(item.getTypeName()+"' object")));
        }
        return new Book_general_foreign(item);
    }

    public static Book_general_foreign newChild(Item parent) {
