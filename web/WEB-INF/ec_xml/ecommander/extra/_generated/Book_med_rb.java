...
public class Book_med_rb
    extends Item
{

    private final static String _ITEM_TYPE_NAME = "book_med_rb";

    private Book_med_rb(Item item) {
        super(item);
    }

    public static Book_med_rb get(Item item) {
        boolean isCompatible = ItemTypeRegistry.getItemPredecessorsExt(item.getTypeName()).contains(_ITEM_TYPE_NAME);
        if (!isCompatible) {
            throw new ClassCastException(("Wrapper 'book_med_rb' can not be created around '"+(item.getTypeName()+"' object")));
        }
        return new Book_med_rb(item);
    }

    public static Book_med_rb newChild(Item parent) {
