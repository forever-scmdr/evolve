...
public class Link_block
    extends Item
{

    public final static String _NAME = "link_block";
    public final static String HEADER = "header";
    public final static String TEXT = "text";
    public final static String LINK = "link";

    private Link_block(Item item) {
        super(item);
    }

    public static Link_block get(Item item) {
        if (item == null) {
            return null;
        }
        boolean isCompatible = ItemTypeRegistry.getItemPredecessorsExt(item.getTypeName()).contains(_NAME);
        if (!isCompatible) {
            throw new ClassCastException(("Wrapper 'link_block' can not be created around '"+(item.getTypeName()+"' object")));
        }
        return new Link_block(item);
    }

    public static Link_block newChild(Item parent) {
