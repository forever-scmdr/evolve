    ...
    extends Item
{
    ...
    public final static String _NAME = "bottom_fixed";
    ...
    private Bottom_fixed(Item item) {
        super(item);
    }

    public static Bottom_fixed get(Item item) {
        if (item == null) {
            return null;
        }
        boolean isCompatible = ItemTypeRegistry.getItemPredecessorsExt(item.getTypeName()).contains(_NAME);
        if (!isCompatible) {
            throw new ClassCastException(("Wrapper 'bottom_fixed' can not be created around '"+(item.getTypeName()+"' object")));
        }
        return new Bottom_fixed(item);
    }

    public static Bottom_fixed newChild(Item parent) {
