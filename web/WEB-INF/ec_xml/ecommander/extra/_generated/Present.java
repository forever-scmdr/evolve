    ...
    extends Item
{
    ...
    private final static String _ITEM_TYPE_NAME = "present";

    private Present(Item item) {
        super(item);
    }

    public static Present get(Item item) {
        if (item == null) {
            return null;
        }
        boolean isCompatible = ItemTypeRegistry.getItemPredecessorsExt(item.getTypeName()).contains(_ITEM_TYPE_NAME);
        if (!isCompatible) {
            throw new ClassCastException(("Wrapper 'present' can not be created around '"+(item.getTypeName()+"' object")));
        }
        return new Present(item);
    }

    public static Present newChild(Item parent) {
