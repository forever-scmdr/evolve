    ...
    extends Item
{
    ...
    public final static String _NAME = "location_block";
    public final static String HEADER = "header";
    public final static String TEXT = "text";
    public final static String LINK = "link";

    private Location_block(Item item) {
        super(item);
    }

    public static Location_block get(Item item) {
        if (item == null) {
            return null;
        }
        boolean isCompatible = ItemTypeRegistry.getItemPredecessorsExt(item.getTypeName()).contains(_NAME);
        if (!isCompatible) {
            throw new ClassCastException(("Wrapper 'location_block' can not be created around '"+(item.getTypeName()+"' object")));
        }
        return new Location_block(item);
    }

    public static Location_block newChild(Item parent) {
