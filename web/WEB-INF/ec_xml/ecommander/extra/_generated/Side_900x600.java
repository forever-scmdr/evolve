...
public class Side_900x600
    extends Item
{

    public final static String _NAME = "side_900x600";
    ...
    private Side_900x600(Item item) {
        super(item);
    }

    public static Side_900x600 get(Item item) {
        if (item == null) {
            return null;
        }
        boolean isCompatible = ItemTypeRegistry.getItemPredecessorsExt(item.getTypeName()).contains(_NAME);
        if (!isCompatible) {
            throw new ClassCastException(("Wrapper 'side_900x600' can not be created around '"+(item.getTypeName()+"' object")));
        }
        return new Side_900x600(item);
    }

    public static Side_900x600 newChild(Item parent) {
