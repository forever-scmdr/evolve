<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:f="f:f" version="2.0">
	...
	<xsl:template match="group">
		...
		<xsl:if test="parameter/value != '' and not($banish = @name)">
			<tr>
				<th colspan="2">
							...
							<form action="{$p/to_cart}" method="post" ajax="true" ajax-loader-id="cart_list_{$p/@id}">
								<xsl:if test="$has_price">
									<input type="number" class="input input_size_lg input_type_number" name="qty"
										   value="{if ($p/min_qty) then f:num($p/min_qty) else 1}" min="{if ($p/min_qty) then f:num($p/min_qty) else 0}" step="{if ($p/step) then f:num($p/step) else 0.1}"/>
