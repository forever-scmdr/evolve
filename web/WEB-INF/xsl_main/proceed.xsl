<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:f="f:f" version="2.0">
	...
	<xsl:template name="TOTAL">
		<div class="cart-total">
			<xsl:if test="f:num(page/cart/sum_discount) != 0">
				<div class="cart-total__text">
					<h3>Сумма заказа</h3>
					Итого: <xsl:value-of select="f:exchange_cur($cart, 'sum_discount', 0)"/>
				</div>
			</xsl:if>
			<xsl:if test="f:num(page/cart/p_sum_discount) != 0">
				<div class="cart-total__text">
					<h3>Сумма предзаказа</h3>
					Итого: <xsl:value-of select="f:exchange_cur($cart, 'p_sum_discount', 0)"/>
				</div>
			</xsl:if>
