<xsl:stylesheet
	...
	<xsl:template name="prices">
		...
		<xsl:variable name="cur"><xsl:choose>
														<xsl:when test="$country = 'BEL'">BYN</xsl:when>
														<xsl:when test="$country = 'RUR'">RUR</xsl:when>
														<xsl:when test="$country = 'EUR'">EUR</xsl:when>
		...
		<table style="display:none" id="{$type}_{$country}" class="prices_table">
			<tr>
				<td></td>
				<td colspan="3">
					Price, <div class="cur_name"><xsl:choose>
														<xsl:when test="$country = 'bel'">BYN</xsl:when>
														<xsl:when test="$country = 'rus'">RUR</xsl:when>
														<xsl:when test="$country = 'eur'">EUR</xsl:when>
													</xsl:choose>
<!-- 					<xsl:value-of select="if ($country = 'bel') then 'BYN' else 'российский рубль'"/> -->
					</div>
			...
			<tr>
				...
				<td class="denom" style="{if ($country = 'bel') then '' else 'display: none'}">
					<select name="" id="sel_denom" onchange="setDays(this, ['first', 'second', 'third'])">
						<xsl:call-template name="number_option"><xsl:with-param name="max" select="number('30')"/></xsl:call-template>
					</select>
				</td>
