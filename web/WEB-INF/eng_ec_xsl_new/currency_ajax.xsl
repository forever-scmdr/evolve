	...
	<xsl:template name="CURRENCY_PLACEHOLDER">
		...
		<script type="text/javascript">
			...
			function selectDefault(container){
				...
				switch(pt){
					case "rus":  $("#currency_converter").find("option[value='RUB']").attr("selected", "selected"); window.curCur = "RUB"; break;
					case "eur": $("#currency_converter").find("option[value='EUR']").attr("selected", "selected"); window.curCur = "EUR"; break;
