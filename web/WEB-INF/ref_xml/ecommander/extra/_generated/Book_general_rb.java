...
public class Book_general_rb
    extends Item
{

    private final static String _ITEM_TYPE_NAME = "book_general_rb";

    private Book_general_rb(Item item) {
        super(item);
    }

    public static Book_general_rb get(Item item) {
        boolean isCompatible = ItemTypeRegistry.getItemPredecessorsExt(item.getTypeName()).contains(_ITEM_TYPE_NAME);
        if (!isCompatible) {
            throw new ClassCastException(("Wrapper 'book_general_rb' can not be created around '"+(item.getTypeName()+"' object")));
        }
        return new Book_general_rb(item);
    }

    public static Book_general_rb newChild(Item parent) {
