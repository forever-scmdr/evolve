...
<xsl:stylesheet
	...
	<xsl:function name="f:exchange_cur">
		...
		<xsl:value-of select="concat($sum, ' ', $currency_out)"/>
	</xsl:function>
	...
	<!-- Удаление переменной из ссылки -->
	<xsl:function name="f:remove_var">
		<xsl:param name="url" as="xs:string?" />
		<xsl:param name="var" as="xs:string?" />

		<xsl:if test="not($var != '') or not($url != '') or not(contains($url, $var))">
			<xsl:sequence select="$url"/>
		</xsl:if>
		<xsl:if test="$var != '' and $url != '' and contains($url, $var)">
			<xsl:variable name="key" select="concat($var, '=')"/>
			<xsl:variable name="before" select="substring-before($url, $key)"/>
			<xsl:variable name="tail" select="substring-after($url, $key)"/>
			<xsl:variable name="after" select="substring-after($tail, '&amp;')"/>

			<xsl:variable name="cut" select="concat($before, $after)"/>
			<xsl:value-of select="if(ends-with($cut, '?')) then substring-before($cut, '?') else $cut" disable-output-escaping="yes"/>
		</xsl:if>
	</xsl:function>
