    ...
    extends Item
{
    ...
    private final static String _ITEM_TYPE_NAME = "employee";

    private Employee(Item item) {
        super(item);
    }

    public static Employee get(Item item) {
        boolean isCompatible = ItemTypeRegistry.getItemPredecessorsExt(item.getTypeName()).contains(_ITEM_TYPE_NAME);
        if (!isCompatible) {
            throw new ClassCastException(("Wrapper 'employee' can not be created around '"+(item.getTypeName()+"' object")));
        }
        return new Employee(item);
    }

    public static Employee newChild(Item parent) {
