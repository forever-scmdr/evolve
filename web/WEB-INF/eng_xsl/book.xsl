<xsl:stylesheet
					...
					<table>
						<tr>
							...
							<td colspan="2">
								...
								<span class="denom" style="{if ($country = 'bel') then '' else 'display: none'}">price before denomination</span>
							</td>
							<td colspan="2">
								Price, <div class="cur_name"><xsl:value-of select="if ($country = 'bel') then 'BYN' else 'RUR'"/></div>
								<span>*- add. seat price</span>
	...
	<div class="common">
		...
		<p>
		<b>Medical voucher:</b> living, feeding and medication.</p>
	    <p><b>Wellness voucher:</b> living, feeding, oxygen cocktails and pool.</p>
    	...
    	<ul class="rooms_menu book"><!-- сделать в главном меню чтобы переходило на последний открытый номер -->
			<li><a class="room_1 menu san open" href="#" onclick="return setPrices('san', currCountry)">Medical voucher</a></li>
			<li><a class="room_2 menu ozd" href="#" onclick="return setPrices('ozd', currCountry)">Wellness voucher</a></li>
		</ul>
		<div class="serv-sec">
			<a class="menu bel open" href="#" onclick="return setPrices(currType, 'bel')">Residents of the Republic of Belarus</a>
			<a class="menu rus" href="#" onclick="return setPrices(currType, 'rus')">Non-residents of the Republic of Belarus</a>
		</div>
