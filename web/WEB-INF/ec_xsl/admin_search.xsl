...
<!--							<th>Мин. заказ</th>-->
							<th>Цена за шт. (<xsl:value-of select="$currency_out" />)</th>
...
<!--			<xsl:if test="$price_byn * $min_qty &lt; f:num(max)">-->
            <xsl:if test="$pack_price_original &lt; f:num(max)">
                ...
                <xsl:if test="$pack_number &gt; 1 or $min_num_double &gt;= 0.5 * $pack_number">
	                <p>
	                    <!--<xsl:value-of select="$min_num_double"/>|--><xsl:if test="$need_sum">x<xsl:value-of select="$number"/>&#160;=&#160;<xsl:value-of select="f:format_currency_precise($sum)"/></xsl:if>
