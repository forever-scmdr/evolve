				...
				<xsl:when test="page/cart/bought and not(page/cart/processed = '1')">
					<form method="post">
						<xsl:for-each select="page/cart/bought">
							...
							<xsl:variable name="price" select="if ($p/price and $p/price != '0') then concat($p/price, ' p.') else 'по запросу'"/>
							<xsl:variable name="sum" select="if ($p/price and $p/price != '0') then concat(sum, ' p.') else ''"/>
							<div class="item">
								...
								<div class="price one"><p><span>Цена</span>
						...
						<div class="total">
							...
							<xsl:if test="not(page/cart/sum != '0')">
								<p>Цена по запросу.</p>
							</xsl:if>
