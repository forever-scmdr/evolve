...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:f="f:f" version="2.0">
		...
		<div class="device items-catalog__device">
			...
			<div class="device__price" style="display:block;">
				<div class="price_normal">
					<xsl:value-of select="concat(f:price_output($price, $shop), ' ', upper-case($curr), ' за ', $min, 'шт')"/>
				</div>
				<div class="nds">*цена включает НДС</div>
				<xsl:if test="count(price_break) &gt; 1">
					<xsl:variable name="x">
						<xsl:for-each select="price_break">
							<xsl:value-of select="concat(f:num(qty), '+ ', '&lt;strong&gt;', f:price_output(price, $shop), ' ', upper-case($curr), '&lt;/strong&gt;', '&lt;br/&gt;')"/>
						</xsl:for-each>
					</xsl:variable>
	...
	<xsl:template match="product" mode="lines"></xsl:template>
	...
	<xsl:template name="CART_BUTTON">
		...
		<div id="cart_list_{$c}">
			<form action="cart_action/?action=addExternalToCart&amp;code={$c}" method="post" ajax="true" ajax-loader-id="cart_list_{$c}">
				...
				<input type="number" class="text-input" name="qty" value="{$min}" min="{$min}" step="if(f:num($step) = 0) then 1 else $step"/>
				<input type="hidden" name="price_map" value="{string-join(($product/price_break/concat(f:num(qty), ':', f:num(price))), ';')}"/>
		...
		<xsl:if test="$available">
			<div class="device__in-stock" style="max-width:140px;"><i class="fas fa-check"></i>поставка<xsl:value-of select="if($qty &lt; 500000) then concat(' ',$qty, ' шт.') else ''" /> в течение <xsl:value-of select="$shop/delivery_string"/></div>
		</xsl:if>
