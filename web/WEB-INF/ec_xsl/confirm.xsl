	...
	<xsl:template name="CONTENT">
		...
		<div class="checkout-cont1">
			<div class="info" style="padding-bottom: 20px;">
				...
				<p></p>
				<p style="margin-top: 20px; font-size: 1.75rem;"><span style="font-weight: bold;">Позиций: </span> <xsl:value-of select="count($cart/bought)"/></p>
			</div>
				...
				<table>
					...
					<xsl:for-each select="$cart/bought">
						...
						<tr>
							...
							<td valign="top">
								<strong>
									<a href="{product/show_product}" target="_blank">
							...
							<td>
								<xsl:value-of select="if(product/price != '0') then product/price else 'цена по запросу'"/>
							</td>
							<xsl:if test="$cart/sum != '0'">
								<td>
