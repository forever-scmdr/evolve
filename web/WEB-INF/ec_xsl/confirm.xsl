<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:f="f:f" version="2.0">
    ...
    <xsl:template name="CONTENT">
        ...
        <h3>Заявка №<xsl:value-of select="$cart/order_num"/>
        </h3>
        <div class="item-summ" style="padding-bottom: 20px;">
            Позиций:
            <xsl:value-of select="count($cart/bought)"/>
            <br/>
            Сумма:
            <span>
                <xsl:value-of select="$cart/sum"/>
            </span>
            руб.
        </div>
            ...
            <div class="info" style="padding-bottom: 20px;">
                <xsl:if test="$is_phys">
                    ...
                    <p>
                        ...
                        <xsl:value-of select="$contacts/address"/>
                    </p>
                    <p>
                        <span style="font-weight: bold;">Дополнительно:</span>&#160;
                        <xsl:value-of select="$contacts/comment"/>
                    </p>
                </xsl:if>
                <xsl:if test="not($is_phys)">
                    ...
                    <p>
                        <span style="font-weight: bold;">Телефон/факс:</span>&#160;
                        <xsl:value-of select="$contacts/phone"/>
                    </p>
                    <p>
                        <span style="font-weight: bold;">Электнонный адрес:</span>&#160;
                        <xsl:value-of select="$contacts/email"/>
                    </p>
                    <p>
                        <span style="font-weight: bold;">Контактное лицо:</span>&#160;
                        <xsl:value-of select="$contacts/contact_name"/>
                    </p>
                    <p>
                        <span style="font-weight: bold;">Телефон контактного лица:</span>&#160;
                        <xsl:value-of select="$contacts/contact_phone"/>
                    </p>
                    <p>
                        <span style="font-weight: bold;">Юр. адрес:</span>&#160;
                        <xsl:value-of select="$contacts/address"/>
                    </p>
                    <xsl:if test="not($contacts/no_account = 'да')">
                        <p>
                            <span style="font-weight: bold;">Расчетный счет:</span>&#160;
                            <xsl:value-of select="$contacts/account"/>
                        </p>
                        <p>
                            <span style="font-weight: bold;">Название банка:</span>&#160;
                            <xsl:value-of select="$contacts/bank"/>
                        </p>
                        <p>
                            <span style="font-weight: bold;">Адрес банка:</span>&#160;
                            <xsl:value-of select="$contacts/bank_address"/>
                        </p>
                        <p>
                            <span style="font-weight: bold;">Код банка:</span>&#160;
                            <xsl:value-of select="$contacts/bank_code"/>
                        </p>
                    </xsl:if>
                    ...
                    <p>
                        ...
                        <xsl:value-of select="$contacts/unp"/>
                    </p>
                    <p>
                        <span style="font-weight: bold;">Ф.И.О директора (индивидуального предпринимателя):</span>&#160;
                        <xsl:value-of select="$contacts/director"/>
                    </p>
                    <p>
                        <span style="font-weight: bold;">Действует на основании:</span>&#160;
                        <xsl:value-of select="$contacts/base"/>
                        <xsl:if test="$contacts/base != 'Устава'">
                            &#160;№
                            <xsl:value-of select="$contacts/base_number"/> от
                            <xsl:value-of select="$contacts/base_date"/>
                        </xsl:if>
                    </p>
                    <p>
                        <span style="font-weight: bold;">Дополнительно:</span>&#160;
