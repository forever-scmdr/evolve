					...
					<table cellspacing="0" cellpadding="0" border="0" width="100%">
						<tr>
							<td style="padding: 40px 40px 0 40px; font-family: sans-serif; font-size: 13px; mso-height-rule: exactly; line-height: 16px; color: #555555; vertical-align: top;">
								<xsl:value-of select="format-date(current-date(), '[D01].[M01].[Y0001]')"/> №<xsl:value-of select="$text/number"/>
							</td>
							<td style="padding: 40px 40px 0 40px; font-family: sans-serif; font-size: 13px; mso-height-rule: exactly; line-height: 16px; color: #555555; vertical-align: top;">
								...
								г. <xsl:value-of select="$agent/city"/><xsl:text> </xsl:text><xsl:value-of select="$agent/address"/><br/>
								<xsl:value-of select="$agent/phone"/><br/>
