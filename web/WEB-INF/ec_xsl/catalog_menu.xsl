...
<xsl:stylesheet
	...
	<xsl:variable name="local_h1" select="if ($proddd) then $proddd/name else if ($sec) then $sec/name else if ($msec) then $msec/name else 'Новинки'"/>
	...
	<div class="container main-content">
		<div class="row">
			<div class="col-sm-4 col-md-3 hidden-xs">
				<div class="h1 no-top-margin">Продукция</div>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<form>
						...
						<div class="form-group">
							<label>Диаметр номинальный DN</label>
							<p id="Dy"></p>
						</div>
						<div class="form-group">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<form>
						...
						<button type="button" onclick="count_a()" class="btn btn-primary btn-block">Подобрать</button>
					</form>
...
<xsl:template name="CATALOG_MARKUP">
	<xsl:if test="page/@name = 'prod' or page/@name = 'sec'">
		<script type="application/ld+json">
			{
				...
				"item": "<xsl:value-of select="concat($base,'/', page/catalog_link)"/>"
			}
			<xsl:if test="$sec">
			,{
				"@context": "http://schema.org",
				...
				"@type": "ListItem",
				"position": 2,
				"name": "<xsl:value-of select="$msec/name"/>",
				"item": "<xsl:value-of select="concat($base,'/',$msec/show_section)"/>"
			}
			<xsl:for-each select="page/catalog//section[.//section/@id = $sec/@id or ($prod and .//@id = $sec/@id)]">
			,{
				"@context": "http://schema.org",
				...
				<!-- "itemListElement": [{ -->
				"@type": "ListItem",
				"position": <xsl:value-of select="position() + 2"/>,
				"name": "<xsl:value-of select="name"/>",
				"item": "<xsl:value-of select="concat($base,'/',show_section)"/>"
			}			
			</xsl:for-each>
			</xsl:if>]
		}
