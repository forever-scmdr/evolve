<xsl:stylesheet
	...
	<xsl:template name="prices">
		...
		<xsl:variable name="cur"><xsl:choose>
														<xsl:when test="$country = 'BEL'">бел. руб.</xsl:when>
														<xsl:when test="$country = 'RUR'">рос. руб.</xsl:when>
														<xsl:when test="$country = 'EUR'">евро</xsl:when>
		...
		<table style="display:none" id="{$type}_{$country}" class="prices_table">
			<tr>
				<td></td>
				<td colspan="3">
					Стоимость, <div class="cur_name"><xsl:choose>
														<xsl:when test="$country = 'bel'">бел. руб.</xsl:when>
														<xsl:when test="$country = 'rus'">рос. руб.</xsl:when>
														<xsl:when test="$country = 'eur'">евро</xsl:when>
													</xsl:choose>
<!-- 					<xsl:value-of select="if ($country = 'bel') then 'бел. руб.' else 'российский рубль'"/> -->
					</div>
					<span>*— с дополнительным местом</span>
				</td>
				<td colspan="3" class="denom" style="{if ($country = 'bel') then '' else 'display: none'}">
						Стоимость бел. руб. до деноминации
						<span>*— с дополнительным местом</span>
				</td>
			</tr>
			<tr>
				...
				<td class="denom" style="{if ($country = 'bel') then '' else 'display: none'}">
					<select name="" id="sel_denom" onchange="setDays(this, ['first', 'second', 'third']); $('#sel_normal').val($(this).val())">
						<xsl:call-template name="number_option"><xsl:with-param name="max" select="number('30')"/></xsl:call-template>
					</select>
				</td>
	...
	<div class="common">
		...
		<div class="room_info">
			...
			<div>
				Для бронирования номера позвоните нам:<br/>
			...
			<div class="r_buttons">
				...
				<a onclick="$('#frm').css('display', 'block');" class="big_button" >Оставить свой отзыв о номере</a>
			</div>
<!-- 			<xsl:value-of select="$room/side_text" disable-output-escaping="yes"/> -->
	...
	<div class="price_room">
		<h3>Стоимость отдыха</h3>
		<div style="display: inline-block">
			<strong>Путёвка</strong>
			<input type="radio" id="san" class="prices_menu" onchange="setPrices('san', currCountry)"/><label for="san">санаторно-курортная</label>
			<input type="radio" id="ozd" class="prices_menu" onchange="setPrices('ozd', currCountry)"/><label for="ozd">оздоровительная</label>
		</div>
		<div style="display: inline-block">
			<strong style="margin-left: 25px;">Гражданство</strong>
			<input type="radio" id="bel" class="prices_menu menu bel" onchange="setPrices(currType, 'bel')"/><label for="bel">для граждан РБ</label>
			<input type="radio" id="rus" class="prices_menu menu rus" onchange="setPrices(currType, 'rus')"/><label for="rus">для иностранцев (RUR)</label>
			<input type="radio" id="eur" class="prices_menu menu eur" onchange="setPrices(currType, 'eur')"/><label for="eur">для иностранцев (EUR)</label>
		</div>
