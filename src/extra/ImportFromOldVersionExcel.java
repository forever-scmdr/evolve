...
public class ImportFromOldVersionExcel extends CreateParametersAndFiltersCommand implements CatalogConst {
	...
	private static final String FILE_UPLOAD_FOLDER = "pictures";
	...
	private void hideAllProducts() throws Exception {
		...
		productsQuery.setParentId(catalog.getId(), true, ItemTypeRegistry.getPrimaryAssoc().getName()).setLimit(100);
		List<Item> products;
		int counter = 0;
		long prevId = 0;
		while ((products = productsQuery.setIdSequential(prevId).loadItems()).size() > 0) {
					...
					while (iterator.hasNext()) {
						...
						if (paramName == null) {
							...
							if (pics.length > 0) {
								...
								String v = currentProduct.getValue(MAIN_PIC_PARAM, "").toString();
	...
	private boolean needNewFile(Item product, String path) {
		...
		File newFile = Paths.get(AppContext.getContextPath(), FILE_UPLOAD_FOLDER, path).toFile();
