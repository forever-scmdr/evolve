...
public class BelchipStrings {
	...
	private static String KEYBOARD_RUSSIAN = "йЙцЦуУкКеЕнНгГшШщЩзЗхХъЪфФыЫвВаАпПрРоОлЛдДжЖэЭяЯчЧсСмМиИтТьЬбБюЮ";
	private static String KEYBOARD_ENGLISH = "qQwWeErRtTyYuUiIoOpP[{]}aAsSdDfFgGhHjJkKlL;:'\"zZxXcCvVbBnNmM,<.>";
	...
	public static String fromRToEKeyboard(String russian) {
        StringBuilder english = new StringBuilder("");
        char[] russianChars = russian.toCharArray();
        for(int i = 0; i < russianChars.length; i++) {
            int alphabetIndex = KEYBOARD_RUSSIAN.indexOf(russianChars[i]);
            if(alphabetIndex != -1)
            	english.append(KEYBOARD_ENGLISH.charAt(alphabetIndex));
            else
            	english.append(russianChars[i]);
        }
        return english.toString();
