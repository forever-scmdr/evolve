		...
		try {
			for (Element productElTree : products) {
				...
				try {
					...
					if (attributes != null) {
						line.append(", [").append(StringEscapeUtils.escapeCsv(attributes.select("> name").text())).append(']');
					...
					if (docs != null) {
						line.append(", [").append(StringEscapeUtils.escapeCsv(docs.select("> name").text())).append(']');
						Element value = docs.getElementsByTag("value").first();
						if (value != null) {
							Elements params = value.getElementsByTag("param");
							for (Element param : params) {
								...
								for (Element docVal : docVals) {
									Element a = docVal.select("a[href]").first();
									if (a != null) {
										if (needSeparator) {
											allDocs.append(";");
										}
										allDocs.append(a.attr("href"));
										needSeparator = true;
									}
								}
					...
					if (env != null) {
						line.append(", [").append(StringEscapeUtils.escapeCsv(env.select(" > name").text())).append(']');
					...
					if (additional != null) {
						line.append(", [").append(StringEscapeUtils.escapeCsv(additional.select("> name").text())).append(']');
