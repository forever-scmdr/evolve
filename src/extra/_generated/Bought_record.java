...
public class Bought_record
    extends Item
{

    private final static String _ITEM_TYPE_NAME = "bought_record";

    private Bought_record(Item item) {
        super(item);
    }

    public static Bought_record get(Item item) {
        if (item == null) {
            return null;
        }
        boolean isCompatible = ItemTypeRegistry.getItemPredecessorsExt(item.getTypeName()).contains(_ITEM_TYPE_NAME);
        if (!isCompatible) {
            throw new ClassCastException(("Wrapper 'bought_record' can not be created around '"+(item.getTypeName()+"' object")));
        }
        return new Bought_record(item);
    }

    public static Bought_record newChild(Item parent) {
