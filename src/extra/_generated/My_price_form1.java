...
public class My_price_form1
    extends Item
{

    private final static String _ITEM_TYPE_NAME = "my_price_form1";

    private My_price_form1(Item item) {
        super(item);
    }

    public static My_price_form1 get(Item item) {
        if (item == null) {
            return null;
        }
        boolean isCompatible = ItemTypeRegistry.getItemPredecessorsExt(item.getTypeName()).contains(_ITEM_TYPE_NAME);
        if (!isCompatible) {
            throw new ClassCastException(("Wrapper 'my_price_form1' can not be created around '"+(item.getTypeName()+"' object")));
        }
        return new My_price_form1(item);
    }

    public static My_price_form1 newChild(Item parent) {
