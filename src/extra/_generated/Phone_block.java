    ...
    extends Item
{
    ...
    public final static String _NAME = "phone_block";
    public final static String HEADER = "header";
    public final static String TEXT = "text";
    public final static String LINK = "link";

    private Phone_block(Item item) {
        super(item);
    }

    public static Phone_block get(Item item) {
        if (item == null) {
            return null;
        }
        boolean isCompatible = ItemTypeRegistry.getItemPredecessorsExt(item.getTypeName()).contains(_NAME);
        if (!isCompatible) {
            throw new ClassCastException(("Wrapper 'phone_block' can not be created around '"+(item.getTypeName()+"' object")));
        }
        return new Phone_block(item);
    }

    public static Phone_block newChild(Item parent) {
