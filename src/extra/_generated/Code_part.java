    ...
    extends Item
{
    ...
    public static Code_part get(Item item) {
        if (item == null) {
            return null;
        }
        boolean isCompatible = ItemTypeRegistry.getItemPredecessorsExt(item.getTypeName()).contains(_NAME);
        if (!isCompatible) {
            throw new ClassCastException(("Wrapper 'code_part' can not be created around '"+(item.getTypeName()+"' object")));
        }
        return new Code_part(item);
    }

    public static Code_part newChild(Item parent) {
