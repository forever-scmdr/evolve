...
public class My_price_from
    extends Item
{

    private final static String _ITEM_TYPE_NAME = "my_price_from";

    private My_price_from(Item item) {
        super(item);
    }

    public static My_price_from get(Item item) {
        if (item == null) {
            return null;
        }
        boolean isCompatible = ItemTypeRegistry.getItemPredecessorsExt(item.getTypeName()).contains(_ITEM_TYPE_NAME);
        if (!isCompatible) {
            throw new ClassCastException(("Wrapper 'my_price_from' can not be created around '"+(item.getTypeName()+"' object")));
        }
        return new My_price_from(item);
    }

    public static My_price_from newChild(Item parent) {
