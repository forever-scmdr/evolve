...
public class Catalog_second
    extends Item
{

    public final static String _NAME = "catalog_second";
    ...
    private Catalog_second(Item item) {
        super(item);
    }

    public static Catalog_second get(Item item) {
        if (item == null) {
            return null;
        }
        boolean isCompatible = ItemTypeRegistry.getItemPredecessorsExt(item.getTypeName()).contains(_NAME);
        if (!isCompatible) {
            throw new ClassCastException(("Wrapper 'catalog_second' can not be created around '"+(item.getTypeName()+"' object")));
        }
        return new Catalog_second(item);
    }

    public static Catalog_second newChild(Item parent) {
