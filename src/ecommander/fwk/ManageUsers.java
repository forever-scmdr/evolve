...
public class ManageUsers extends DBPersistenceCommandUnit implements ErrorCodes {
	...
	public void execute() throws Exception {
		if (mode == CREATE) {
			...
			if(StringUtils.isNotBlank(password)) {
				try {
					executeCommand(new SaveNewUserDBUnit(newUser).ignoreUser(ignoreUser));
	...
	private void makeRegistered() throws Exception {
		Parameter regParam = userItem.getParameterByName(REGISTERED_GROUP);
		if (regParam != null && regParam.hasChanged() && userItem.getByteValue(REGISTERED_GROUP) != (byte) 1) {
			userItem.setValue(REGISTERED_GROUP, (byte) 1);
		...
		executeCommand(ChangeItemOwnerDBUnit.newUser(userItem, newUser.getUserId(), UserGroupRegistry.getGroup(REGISTERED_GROUP)));
