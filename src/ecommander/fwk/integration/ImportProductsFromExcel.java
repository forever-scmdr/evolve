...
public class ImportProductsFromExcel extends CreateParametersAndFiltersCommand implements CatalogConst {
	...
	private boolean hasAuxParams = false;
...
//										product.clearValue("medium_pic");
			...
			private void createExtraPages(Item product) throws Exception {
				...
				int extraCount = 0;
				try {
					String  extras = getValue(CreateExcelPriceList.EXTRA_COLS);
					extraCount = Integer.parseInt(extras);
				} catch (Exception e) {}
...
//				}
			...
			private Item ensureParamsXML(Item product) throws Exception {
				...
				return (paramsXML == null) ? Item.newChildItem(paramsXMLItemType, product) : paramsXML;
			}
