...
//			String orderNumber = count + "-" + newDate;
    ...
    public ResultPE createComplectation() throws Exception {
        String idStr = getVarSingleValue(PROD_PARAM);
        double quantity = 0;
        long prodId = 0;
        try {
        ...
        checkStrategy();
        ensureCart();
        Item bought = createBought(prodId, quantity);
        boolean complectationSaved = saveOptionsToComplectation(bought, prodId);
    ...
    private boolean saveOptionsToComplectation(Item bought, long prodId) throws Exception {
        String priceParam = getInitiator().inGroup("registered") ? "price_opt" : "price";
        ...
        HashMap<Long, Integer> idQtys = new HashMap<>();
        for (Object q : input) {
            String varName = "qty-" + q.toString();
            int qty = Integer.parseInt(getVarSingleValueDefault(varName, "1"));
            idQtys.put(Long.parseLong(q.toString()), Math.max(qty, 1));
        }
        List<Item> allOptions = ItemQuery.loadByParentId(prodId, new Byte[]{new Byte(ItemTypeRegistry.getAssocId("option"))});
    ...
    private boolean isValidOptionQuantities(HashMap<Long, Integer> idQtys, List<Item> allOptions) {
        Map<Long, Item> map = allOptions.stream().collect(Collectors.toMap(x -> x.getId(), x -> x));
        for (long id : idQtys.keySet()) {
            Item item = map.get(id);
            if (item == null) return false;
            if (item.getIntValue("max", Integer.MAX_VALUE) < idQtys.get(id)) {
                return false;
            }
        }
        return true;
    ...
    private boolean isValidOptionList(List<Long> ids, List<Item> allOptions) {
        HashMap<String, Set<Long>> mandatoryOptionGroups = new HashMap<>();
        HashMap<String, Set<Long>> optionalOptionGroups = new HashMap<>();

        //populate option maps
        for (Item option : allOptions) {
            String group = option.getStringValue("group", "");
            boolean mandatory = option.getByteValue("mandatory", (byte) 0) == 1;
            long id = option.getId();
            HashMap<String, Set<Long>> map = mandatory ? mandatoryOptionGroups : optionalOptionGroups;
            Set<Long> idSet = map.get(group);
            if (idSet == null) {
                idSet = new HashSet<>();
                map.put(group, idSet);
            }
        ...
        for (String key : mandatoryOptionGroups.keySet()) {
            Set<Long> idSet = mandatoryOptionGroups.get(key);
            int n = 0;
            for (Long id : idSet) {
                if (ids.contains(id)) {
                    n++;
            ...
            //incomparable options selected
            if (StringUtils.isNotBlank(key) && n > 1) return false;
            //mandatory option not selected
            if (n == 0) return false;
        }

        //check optional options
        for (String key : optionalOptionGroups.keySet()) {
