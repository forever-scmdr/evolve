...
public abstract class Command {
	...
	protected final LinkedHashMap<Long, Item> getLoadedItems(String itemPageId) {
		...
		AllFoundIterator iter = page.getItemPEById(itemPageId).getAllFoundItemIterator();
		while (iter.next()) {
	...
	protected final ResultPE getResult(String name) throws EcommanderException {
		...
		if (result == null)
			return new ResultPE(name, ResultType.redirect);
		return result;
	}
	/**
	 ...
	 * @return
	 */
	protected final ResultPE getResultingUrl(String url, ResultType type) {
