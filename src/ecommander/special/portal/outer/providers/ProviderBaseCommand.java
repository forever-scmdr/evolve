...
public class ProviderBaseCommand extends Command implements ItemNames {
    ...
    protected ProviderBaseCommand.Input inp; // данные, полученные от пользователя и другие внешние данные
    protected BigDecimal globalMaxPrice = BigDecimal.ONE.negate(); // максимальная цена (для создания итогового документа)
    protected BigDecimal globalMinPrice = BigDecimal.valueOf(Double.MAX_VALUE); // минимальная цена (для создания итогового документа)


    public ProviderBaseCommand(Command outer) {
        super(outer);
    }

    protected class Input {
        public final List<Object> servers;
        public final String curCode;
        public final String query;
        public final BigDecimal fromFilterDecimal;
        public final BigDecimal toFilterDecimal;
        public final boolean hasShipDateFilter;
        public final boolean hasVendorFilter;
        public final boolean hasDistributorFilter;
        public final boolean hasFromFilter;
        public final boolean hasToFilter;
        public final HashSet<Object> shipDateFilter;
        public final HashSet<Object> vendorFilter;
        public final HashSet<Object> distributorFilter;
        public final HashSet<Object> dstrSet;
        public final BigDecimal globalMaxPrice;
        public final BigDecimal globalMinPrice;

        private ResultPE errorResult;

        private CurrencyRates rates = null;

        private Input() throws EcommanderException {
            ...
            if (StringUtils.isBlank(query)) {
                errorResult = getResult("illegal_argument").setValue("Неверный формат запроса");
            }
            this.query = query;
    ...
    public ResultPE execute() throws Exception {
        inp = new Input();
        if (inp.errorResult != null) {
            return inp.errorResult;
        }
    ...
    protected void performLocalSearch(XmlDocumentBuilder xml) throws Exception {
        // Загрузка курсов валют
        String priceParamName = PRICE_PREFIX + (StringUtils.isBlank(inp.curCode) ? inp.getRates().getDefaultCurrency() : inp.curCode);
        ...
        for (Item prod : prods) {
            ...
            if (!StringUtils.equals(distributor, currentDistributor)) {
                ...
                Item catalogSettings = new ItemQuery(PRICE_CATALOG).addParameterEqualsCriteria(ItemNames.price_catalog_.NAME, distributor).loadFirstItem();
