...
public class CleanDeletedItemsDBUnit extends DBPersistenceCommandUnit implements DBConstants.ItemTbl, DBConstants.ItemParent {
	...
	public void execute() throws Exception {
		...
		selectToDeleteIds.SELECT(I_ID).FROM(I_TABLE).WHERE().col(I_STATUS).setByte(Item.STATUS_DELETED).
		...
		delete.DELETE(I_TABLE).WHERE().col(I_ID, " IN(").setLongArray(deletedArray).sql(");\r\n")
				.DELETE(IP_TABLE).WHERE().col(IP_CHILD_ID, " IN(").setLongArray(deletedArray).sql(")");
