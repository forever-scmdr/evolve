...
public class NonemptyEmailCommand extends Command {
	...
	public ResultPE execute() throws Exception {
		...
		try {
			...
			if(hasTemplate && "yes".equals(getVarSingleValue("debug_template"))) {
				ResultPE debugPage = getResult("debug");
				for (ParameterDescription param : postDesc.getParameterList()) {
					...
					if(paramDesc.getDataType().isFile()) continue;
					String varName = param.getName();
					Object value = postForm.getValue(param.getId());
					ArrayList<String> strings = new ArrayList<String>();
					
					if(value != null && value.getClass().isAssignableFrom(ArrayList.class)) {
						strings = (ArrayList<String>)value;
						for(String s : strings) {
							if(StringUtils.isNotBlank(s)) {
								debugPage.addVariable(varName, s);
							}
						}
					}
					if(value != null){
						debugPage.addVariable(varName, value.toString());
					}
				}
				return debugPage;
			}
				...
				if (paramDesc.getDataType().isFile()) {
					...
					if (hasTemplate) {
						...
						if(value != null && value.getClass().isAssignableFrom(ArrayList.class)) {
							...
							for(String s : strings) {
								if(StringUtils.isNotBlank(s)) {
									emailPage.addVariable(new StaticVariablePE(varName, s));
								}
							}
						}
						else if(value != null){
							emailPage.addVariable(new StaticVariablePE(varName, value.toString()));
						}
