...
public class Top_728x90
    extends Item
{

    public final static String _NAME = "top_728x90";
    ...
    private Top_728x90(Item item) {
        super(item);
    }

    public static Top_728x90 get(Item item) {
        if (item == null) {
            return null;
        }
        boolean isCompatible = ItemTypeRegistry.getItemPredecessorsExt(item.getTypeName()).contains(_NAME);
        if (!isCompatible) {
            throw new ClassCastException(("Wrapper 'top_728x90' can not be created around '"+(item.getTypeName()+"' object")));
        }
        return new Top_728x90(item);
    }

    public static Top_728x90 newChild(Item parent) {
