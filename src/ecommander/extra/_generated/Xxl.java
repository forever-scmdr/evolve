    ...
    extends Item
{
    ...
    public final static String _NAME = "xxl";
    public final static String HEADER = "header";
    public final static String TEXT = "text";
    public final static String TEXT_PIC = "text_pic";

    private Xxl(Item item) {
        super(item);
    }

    public static Xxl get(Item item) {
        if (item == null) {
            return null;
        }
        boolean isCompatible = ItemTypeRegistry.getItemPredecessorsExt(item.getTypeName()).contains(_NAME);
        if (!isCompatible) {
            throw new ClassCastException(("Wrapper 'xxl' can not be created around '"+(item.getTypeName()+"' object")));
        }
        return new Xxl(item);
    }

    public static Xxl newChild(Item parent) {
