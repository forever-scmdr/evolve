    ...
    extends Item
{
    ...
    private final static String _ITEM_TYPE_NAME = "contact_form";

    private Contact_form(Item item) {
        super(item);
    }

    public static Contact_form get(Item item) {
        boolean isCompatible = ItemTypeRegistry.getItemPredecessorsExt(item.getTypeName()).contains(_ITEM_TYPE_NAME);
        if (!isCompatible) {
            throw new ClassCastException(("Wrapper 'contact_form' can not be created around '"+(item.getTypeName()+"' object")));
        }
        return new Contact_form(item);
    }

    public static Contact_form newChild(Item parent) {
