    ...
    extends Item
{
    ...
    private final static String _ITEM_TYPE_NAME = "pic";

    private Pic(Item item) {
        super(item);
    }

    public static Pic get(Item item) {
        boolean isCompatible = ItemTypeRegistry.getItemPredecessorsExt(item.getTypeName()).contains(_ITEM_TYPE_NAME);
        if (!isCompatible) {
            throw new ClassCastException(("Wrapper 'pic' can not be created around '"+(item.getTypeName()+"' object")));
        }
        return new Pic(item);
    }

    public static Pic newChild(Item parent) {
