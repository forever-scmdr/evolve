 ...
 * 
 * @author EEEE TODO <enhance> переделать для новой версии сервлетов TODO <fix> Сделать удаление старых файлов при замене их на новые (сейчас
 *         отсутствует). Загружать старые значения из StringIndex, вызывать эту команду до апдейта параметров в индексе
 */
public class SaveItemFilesUnit extends ItemFileUnit {
	...
	/**
	 * Перемещает файлы из директории загрузки в постоянную директорию и дает им новое название (c ID предков) Также устанавливает новое название
	 * в соответствующем параметре айтема
	 */
	...
	/**
	 * Устанавливает старое значение для всех парамтеров файлов (без ID предков) и удаляет файл из директории для файлов
	 */
	...
	public static void main(String[] args) {
		try {
			String q = "http://экокожа.бел/img/picture4.jpg";
			Pattern urlPattern = Pattern.compile("^(https?|ftp|file)://[-\\wА-Яа-я+&@#/%?=~|!:,.;]*[-\\wА-Яа-я+&@#/%=~|]");
			Matcher m = urlPattern.matcher((CharSequence) q);
			System.out.println(m.matches());
			URL url = new URL(q);

			String human = URLDecoder.decode(url.getPath(), "UTF-8");
