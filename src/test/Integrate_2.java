...
import ecommander.persistence.commandunits.DeleteItemBDUnit.DeleteInformer;
 ...
 * 
 */
public class Integrate_2 extends Command implements DeleteInformer {
		...
		if (isInProgress || StringUtils.isEmpty(op) || !op.equals("start")) {
			...
			synchronized (MUTEX) {
				...
				try {
					...
					List<Item> catalogs = ItemQuery.newItemQuery(ItemNames.CATALOG._ITEM_NAME).loadItems();
					if (catalogs.size() != 1) {
					...
					integration = new File(AppContext.getFilesDirPath() + catalog.getPredecessorsAndSelfPath()
							+ catalog.getStringValue("integration"));
					...
					new Thread(new Runnable(){
						public void run() {
							try {
								...
								// Удалить все разделы
								List<Item> sections = ItemQuery.newItemQuery(ItemNames.SECTION_FIRST._ITEM_NAME).setPredecessorId(catalog.getId()).loadItems();
								for (Item section : sections) {
									integrate.deletedBase += integrate.deletedCount;
									integrate.deletedCount = 0;
									DeleteItemBDUnit delete = new DeleteItemBDUnit(section);
									delete.setInformer(integrate);
